...
class AudioFinetuningConfig(AudioPretrainingConfig):
    ...
    adapters_only: bool = field(
        default=False, metadata={"help": "Finetune only adapters and layer normalization and freeze the rest of the network."}
    )
...
class AudioFinetuningTask(AudioPretrainingTask):
    ...
    def build_model(self, model_cfg: FairseqDataclass, from_checkpoint=False):
        ...
        if self.cfg.adapters_only:
            for name, w in model.state_dict().items():
                w.require_grad = 'layernorm' in name.lower() or 'adapter' in name.lower()
